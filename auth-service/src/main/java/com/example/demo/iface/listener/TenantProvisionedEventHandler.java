        ...
        try {
            ...
            if (!idempotencyHandler.tryProcess(payload.eventId().toString())) {
                log.debug("[EDA] 忽略已處理的重複租戶開通事件: EventId={}, TenantId={}",
                        payload.eventId(), payload.tenantId());
                return;
            }

            log.info("[EDA] 接收到新租戶開通事件，準備初始化 Root Admin。TenantId: {}", payload.tenantId());

            // 2. 跨越六角架構邊界，將流程編排委託給 Application Layer
            onboardingCommandService.initializeTenantRootSecurity(
                    payload.tenantId(),
                    payload.rootAdminEmail(),
                    payload.rootAdminTempPassword()
            );

            log.info("[EDA] 租戶 {} 的 Root Admin 初始化流程執行完畢。", payload.tenantId());
